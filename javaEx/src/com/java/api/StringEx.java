...
//		stringMethods();
	...
	private static void stringBufferEx() {
		...
		//	-> 내부에 버퍼(임시 저장공간)를 가진 StringBuffer를 활용하면 메모리 관리에 용이
	...
	private static void stringMethods() {
		...
		System.out.println("m의 위치 인덱스 2nd: " + s.indexOf("m", 12));	//	12번 인덱스부터 다시 검색
		
		//	문자열 추출: substring
		System.out.println("substring (끝까지) : " + s.substring(5));	//	5부터 끝까지
		System.out.println("substring (범위) : " + s.substring(5, 16));	//	 5부터 16까지
		
		//	치환
		System.out.println("치환 : " + s.replace('?', '!'));
		System.out.println("모두 치환 : " + 
					s.replaceAll("Fun", "Funny").replace('?', '!'));	//	메서드 체이닝
...
//		System.out.println(word1 + word2);
		...
		//	ABC < ABD
		//	같으면 0, 비교하려는 값이 작으면 음수, 비교하려는 값이 크면 양수
